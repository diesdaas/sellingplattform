 ...
 */
export async function getVendorProfileByUsername(req, res, next) {
  try {
...
export async function registerVendor(req, res, next) {
  try {
    ...
    const { storeName, storeDescription, username, address, contact, logo, email } = req.body;
    ...
    const storeResult = await query(
      `INSERT INTO "Store" (id, "userId", name, description, username, address, contact, logo, email, status, "isActive", "createdAt", "updatedAt")
       ...
       RETURNING *`,
      [userId, storeName, storeDescription || '', username, address || '', contact || '', logo || req.user.image || '', email || req.user.email]
    );
